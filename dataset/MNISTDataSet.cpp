...
int ReverseInt (int i)  // the integers are store in MSB first manner
{
...
std::pair<std::vector<VectorXd>, std::vector<double>> MNISTDataSet::loadDataSet(std::string label_path, std::string image_path)
{
    // read MNIST data
    ...
    {
        std::cerr << "Label file has wrong magic number: " << label_magicNumber << " expected: " << LABEL_MAGIC << std::endl;
    }
    ...
    {
        std::cerr << "Image file has wrong magic number: " <<  image_magicNumber << " expected: " << IMAGE_MAGIC << std::endl;
    }
    ...
    label_stream.close();
    image_stream.close();
    return std::make_pair(image_list, label_list);
}
