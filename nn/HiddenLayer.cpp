...
vector<double> HiddenLayer::forwardPropagation(VectorXd input, set<int> nn_node_set, bool training, NN_parameters m_theta)
{
    ...
    for (auto& idx : nn_node_set)
    {
        ...
        for(int i = 0; i < input.size(); i ++)
            prod += m_theta.getWeightVector(m_pos, idx)[i] * input(i);
        m_weightedSum[idx] = prod + m_theta.getBias(m_pos, idx); // input.transpose() ????????? getWeightVector returns vector<double> instead of VectorXd
    }
    return activationFunction();
}

vector<double> HiddenLayer::forwardPropagation(vector<double> input, NN_parameters m_theta)
{
    return forwardPropagation(vectorize(input), false, m_theta);
}

vector<double> HiddenLayer::forwardPropagation(vector<double> input, bool training, NN_parameters m_theta)
{
...
vector<double> HiddenLayer::forwardPropagation(VectorXd input, bool training, NN_parameters m_theta)
{
    return forwardPropagation(input, m_theta.retrieveNodes(m_pos, input), training, m_theta);
}

vector<double> HiddenLayer::forwardPropagation(VectorXd input, vector<int> hashes, bool training, NN_parameters m_theta)
{
    return forwardPropagation(input, m_theta.retrieveNodes(m_pos, hashes), training, m_theta);
}

vector<double> HiddenLayer::calculateDelta(vector<double> prev_layer_delta, NN_parameters m_theta)
{
...
{
    cout << m_pos << " : " << m_total_nn_set_size / size;
}


VectorXd HiddenLayer::vectorize(vector<double> data)
{
    return vectorize(data, 0, data.size());
}

VectorXd HiddenLayer::vectorize(vector<double> data, int offset, int length)
{
